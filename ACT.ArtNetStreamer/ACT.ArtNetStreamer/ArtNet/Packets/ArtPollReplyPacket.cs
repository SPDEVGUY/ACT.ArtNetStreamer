...
namespace ArtNet.Packets
{
    ...
    public class ArtPollReplyPacket : ArtNetPacket
    {
        ...
        public override string ToString()
        {
            ...
            sb.AppendLine($"IpAddress:{IpAddress[0]}.{IpAddress[1]}.{IpAddress[2]}.{IpAddress[3]}");
            sb.AppendLine($"Port:{Port}");
            sb.AppendLine($"FirmwareVersion:{FirmwareVersion}");
            sb.AppendLine($"SubSwitch:{SubSwitch}");
            sb.AppendLine($"Oem:{Oem}");
            sb.AppendLine($"UbeaVersion:{UbeaVersion}");
            sb.AppendLine($"Status = {Status}");
            sb.AppendLine($"EstaCode = {EstaCode}");
            sb.AppendLine($"ShortName = {ShortName}");
            sb.AppendLine($"LongName = {LongName}");
            sb.AppendLine($"NodeReport = {NodeReport}");
            sb.AppendLine($"PortCount = {PortCount}");
            sb.AppendLine($"PortTypes = {PortTypes}");
            sb.AppendLine($"GoodInput = {GoodInput}");
            sb.AppendLine($"GoodOutput = {GoodOutput}");
            sb.AppendLine($"SwIn = {SwIn}");
            sb.AppendLine($"SwOut = {SwOut}");
            sb.AppendLine($"SwVideo = {SwVideo}");
            sb.AppendLine($"SwMacro = {SwMacro}");
            sb.AppendLine($"SwRemote = {SwRemote}");
            sb.AppendLine($"Style = {Style}");
            sb.AppendLine($"MacAddress = {MacAddress}");
            sb.AppendLine($"BindIpAddress = {BindIpAddress[0]}.{BindIpAddress[1]}.{BindIpAddress[2]}.{BindIpAddress[3]}");
            sb.AppendLine($"BindIndex = {BindIndex}");
